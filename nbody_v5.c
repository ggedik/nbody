                    ...
                    u64 n) //just this part to translate 
{
  ...
  const f32 softening = 1e-20;  //what does it do? 
      ...
      //f32 *fx = (f32*) aligned_alloc(16,sizeof(f32) * 4);  //do it in a smart way 
      //f32 *fy = (f32*) aligned_alloc(16,sizeof(f32) * 4);
      //f32 *fz = (f32*) aligned_alloc(16,sizeof(f32) * 4);
      ...
      //f32 *tmpx = aligned_alloc(16, sizeof(f32) *4);
      //f32 *tmpy = aligned_alloc(16, sizeof(f32) *4);
      //f32 *tmpz = aligned_alloc(16, sizeof(f32) *4);
f32 tmpx; 
f32 tmpy; 
f32 tmpz;
f32 fx, fy, fz; 
  ...
  for (u64 i = 0; i < n; i++){ //for every particle 
    ...
    //23 floating-point operations
      for (u64 j = 0; j < n; j=j+4){
	       //Newton's law
	      dx[0] = p.x[j]   - tmpx;//1 (sub) //how much a particle changed position
	      dx[1] = p.x[j+1] - tmpx;//
        dx[2] = p.x[j+2] - tmpx;//
        dx[3] = p.x[j+3] - tmpx;//
      
        dy[0] = p.y[j]   - tmpy;///2 (sub)
        dy[1] = p.y[j+1] - tmpy;//
        dy[2] = p.y[j+2] - tmpy;//
        dy[3] = p.y[j+3] - tmpy;//
  ...
  for (u64 i = 0; i < n; i=i+4){ //vectorize them with a filler 
      p.x[i]   += dt * p.vx[i]; 
      p.x[i+1] += dt * p.vx[i+1];
      p.x[i+2] += dt * p.vx[i+2];
      p.x[i+3] += dt * p.vx[i+3];
      ...
      p.z[i+1] += dt * p.vz[i+1];
      p.z[i+2] += dt * p.vz[i+2];  
      p.z[i+3] += dt * p.vz[i+3]; //48 bytes total 3full R-- registers=>16bytes
    }
}

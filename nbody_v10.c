//ASSEMBLY VECTORIZED SIMD 
/*
SSE REGISTERS => 16 bytes => 4 floats 
*packed instructions and scalar instructions  */
#include <omp.h>
...
{
  //
  const u64 n     = (argc > 1) ? atoll(argv[1]) : 16384; //number of particles
  const u64 steps = 10;         
  const f32 dt    = 0.01;
